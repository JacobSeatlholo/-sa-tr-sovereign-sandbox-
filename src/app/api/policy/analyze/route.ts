...
import { complete, AIServiceUnavailableError } from "@/lib/ai";
import { incrementPolicyAnalyses } from "@/lib/store";
...
export async function POST(req: Request) {
  ...
  const userPrompt = `Analyse the following official text for the ${direction === "TR-EN" ? "Turkish → English" : "English → Turkish"} bilateral channel.
...
--- END OFFICIAL TEXT ---`;
  ...
  try {
    ...
    if (!analysis) {
      // Model returned unparseable output — degrade gracefully to raw text.
      const fallback: PolicyAnalysis = {
        translation: raw.trim(),
        summary: "Structured parsing unavailable for this input; raw rendition returned.",
        commitments: [],
        domains: [],
        riskFlags: [],
      };
      incrementPolicyAnalyses();
      return NextResponse.json({ ok: true, data: fallback, degraded: true });
    }
    ...
    return NextResponse.json({
      ...
      data: analysis,
      degraded: false,
      meta: { processingMs: Date.now() - started, direction },
    });
  } catch (err) {
